...
def make_pred_multilabel(
    ...
    for i, data in tqdm(enumerate(dataloader)):
        ...
        inputs, labels = inputs.cuda(opt.gpu_ids[0]), labels.cuda(opt.gpu_ids[0])
    ...
    pred_df.to_csv(os.path.join(opt.run_path, "preds.csv"), index=False)
    auc_df.to_csv(os.path.join(opt.run_path, "aucs.csv"), index=False)
